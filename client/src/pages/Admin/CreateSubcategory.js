...
const CreateSubcategory = () => {
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        const response = await axios.get('http://localhost:8087/api/v1/category/get-category');
  ...
  const handleSubcategoryCreation = async () => {
    try {
      ...
      await axios.post('http://localhost:8087/api/v1/category/create-subcategory', {
        subcategoryName,
        selectedCategory,
      });
